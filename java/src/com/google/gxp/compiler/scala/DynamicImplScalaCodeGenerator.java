...
/**
 * A {@code CodeGenerator} which generates Scala code in dynamic
 * impl mode.
 */
public class DynamicImplScalaCodeGenerator extends ScalaCodeGenerator {
  private final long compilationVersion;

  public DynamicImplScalaCodeGenerator(MessageExtractedTree tree,
  ...
  protected static class TemplateWorker
      extends ScalaCodeGenerator.TemplateWorker {
    private final long compilationVersion;
    ...
    private class StatementVisitor extends ScalaCodeGenerator.TemplateWorker.StatementVisitor {
      ...
      public Void visitExtractedMessage(ExtractedMessage msg) {
        ...
        for (Expression param : params) {
          ...
          formatLine(param.getSourcePosition(),
                     ...
                     + ".toString();",
                     varName, msg.getSchema().getScalaAppender(), getScalaExpression(param));
        }
    ...
    private class ToEscapableExpressionVisitor
        extends ScalaCodeGenerator.TemplateWorker.ToEscapableExpressionVisitor {

      @Override
      public String visitExtractedMessage(ExtractedMessage msg) {
        StringBuilder sb = new StringBuilder("formatGxpMessage(");
        sb.append(SCALA.toStringLiteral(msg.getTcMessage().getOriginal()));
        for (Expression param : msg.getParameters()) {
          sb.append(", ");
          sb.append(msg.getSchema().getScalaAppender());
          sb.append(".INSTANCE.append(new StringBuilder(), gxp_context, ");
          sb.append(getScalaExpression(param));
          sb.append(").toString()");
        }
        sb.append(')');
        return sb.toString();
