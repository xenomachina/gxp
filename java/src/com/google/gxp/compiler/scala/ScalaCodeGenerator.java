...
/**
 * A {@code CodeGenerator} that generates Scala code.
 */
public class ScalaCodeGenerator extends BaseScalaCodeGenerator<MessageExtractedTree> {
   ...
   */
  public ScalaCodeGenerator(MessageExtractedTree tree, String runtimeMessageSource) {
   ...
   */
  protected static class TemplateWorker extends BaseScalaCodeGenerator.TemplateWorker {
    ...
    protected void appendClass() {
      ...
      if (runtimeMessageSource != null) {
        formatLine("private static final String GXP$MESSAGE_SOURCE = %s;",
                   SCALA.toStringLiteral(runtimeMessageSource));
    ...
    private String getAnonymousScalaType(Schema schema) {
      return "Anonymous" + getBaseName(schema.getScalaType());
    }
    ...
    private void appendAnonymousInterfaces() {
      for (Schema schema : anonymousSchemas) {
        appendLine();
        appendLine("private abstract static class " + getAnonymousScalaType(schema));
        appendLine("    extends GxpTemplate.AnonymousGxpClosure");
        appendLine("    implements " + schema.getScalaType() + " {");
        appendLine("}");
      }
    }
      ...
      for (Parameter param : template.getAllParameters()) {
        ...
        if (defaultValue != null) {
          ...
          formatLine(param.getDefaultValue().getSourcePosition(),
                     ...
                     getScalaExpression(param.getDefaultValue()));
    ...
    private static final int MAX_SCALA_STRING_LENGTH = 65534;
    ...
    protected void writeString(SourcePosition pos, String s) {
      ...
      if (length != 0) {
        int curPos = 0;
        while (length - curPos > MAX_SCALA_STRING_LENGTH) {
          writeExpression(pos, SCALA.toStringLiteral(
                              s.substring(curPos, curPos + MAX_SCALA_STRING_LENGTH)));
          curPos += MAX_SCALA_STRING_LENGTH;
        }
        writeExpression(pos, SCALA.toStringLiteral(s.substring(curPos, length)));
      }
    }
    ...
    /**
     * A visitor that creates Alerts when it encounters a duplicate Scala message name.
     */
    ...
    private class NamedExtractedMessageVisitor extends ExhaustiveExpressionVisitor {
      ...
      private void validateName(ExtractedMessage msg) {
        SCALA.validateName(alertSink, msg, msg.getName(SCALA));
      }
    ...
    protected class StatementVisitor extends DefaultingExpressionVisitor<Void>
      ...
      public Void visitExceptionExpression(ExceptionExpression value) {
        ...
        appendLine(value.getSourcePosition(),
                   "throw new " + excClass + "("
                   + SCALA.toStringLiteral(value.getMessage()) + ");");
        return null;
      }
      ...
      public Void visitEscapeExpression(EscapeExpression value) {
        formatLine(value.getSourcePosition(), "%s.INSTANCE.append(%s, %s, %s);",
                   value.getSchema().getScalaAppender(), GXP_OUT_VAR, GXP_CONTEXT_VAR,
      ...
      public Void visitLoopExpression(LoopExpression loop) {
        ...
        if (loop.getIterator() != null && loop.getIterator().canEvaluateAs(SCALA)) {
          ...
          formatLine(loop.getSourcePosition(),
                     "while (%s.hasNext()) {",
                     iterVar);
          String itemExpr = ScalaUtil.unbox(iterVar + ".next()",
                                            toScalaType(loop.getType()));
          formatLine(loop.getType().getSourcePosition(),
                     "final %s %s = %s;",
                     toScalaType(loop.getType()),
                     SCALA.validateName(alertSink, loop, loop.getVar()),
                     itemExpr);
          ...
          appendLine("}");
        } else if (loop.getIterable() != null && loop.getIterable().canEvaluateAs(SCALA)) {
          formatLine(loop.getSourcePosition(), "for (final %s %s : %s) {",
                     toScalaType(loop.getType()),
                     SCALA.validateName(alertSink, loop, loop.getVar()),
                     getScalaExpression(loop.getIterable()));
          writeConditionalDelim(delimiter, boolVar);
          if (keyVar != null) {
            formatLine("final int %s = %s++;", keyVar, tmpKeyVar);
          }
      ...
      public Void visitAbbrExpression(AbbrExpression abbr) {
        ...
        formatLine(abbr.getSourcePosition(),
                   "final %s %s = %s;",
                   toScalaType(abbr.getType()),
                   SCALA.validateName(alertSink, abbr, abbr.getName()),
                   getScalaExpression(abbr.getValue()));
          ...
          public Boolean visitInstanceCallable(InstanceCallable callable) {
            ...
            if (thisAttr != null) {
              formatLine(call.getSourcePosition(), "%s %s = %s;",
                         toScalaType(callable.getInstanceType()),
    ...
    protected final String getScalaExpression(Expression value) {
      return value.acceptVisitor(toExpressionVisitor);
    }

    protected final String getEscapedString(Expression value) {
      StringBuilder sb = new StringBuilder(value.getSchema().getScalaAppender());
      sb.append(".INSTANCE.append(new java.lang.StringBuilder(), ");
      sb.append(GXP_CONTEXT_VAR);
      sb.append(", ");
      sb.append(getScalaExpression(value));
      sb.append(").toString()");
      return sb.toString();
    }
    ...
    /**
     * Converts an Expression into a Scala expression that evaluates to a String
    ...
    protected class ToEscapableExpressionVisitor
      ...
      public String visitNativeExpression(NativeExpression value) {
        ...
        sb.append('(');
        sb.append(SCALA.validateExpression(alertSink, value));
        sb.append(')');
        return sb.toString();
      }
      ...
      public String visitEscapeExpression(EscapeExpression value) {
        StringBuilder sb = new StringBuilder();
        sb.append(value.getSchema().getScalaAppender());
      ...
      public String visitValidatedCall(ValidatedCall call) {
        ...
        sb.append(".getString(");
        sb.append(getScalaExpression(call));
        sb.append(")");
        return sb.toString();
      }
    }
    ...
    private class ToExpressionVisitor
      ...
      public String visitAttrBundleParam(AttrBundleParam bundle) {
        ...
        sb.append(bundle.getSchema().getScalaType());
        sb.append(">(");
        List<String> includeAttrs = Lists.newArrayList();
        for (String includeAttr : bundle.getIncludeAttrs()) {
          includeAttrs.add(SCALA.toStringLiteral(includeAttr));
        }
      ...
      public String visitObjectConstant(ObjectConstant node) {
        ...
        String type = toScalaType(node.getType());
        if (!ScalaUtil.isPrimitiveType(type)) {
          value = type + ".valueOf(\""
              + CharEscapers.javaStringEscaper().escape(value) + "\")";
        } else {
          if (!ScalaUtil.isValidPrimitive(value, type)) {
            alertSink.add(new IllegalScalaPrimitiveError(node, value, type));
          }
        }
      ...
      @Override
      public String visitNativeExpression(NativeExpression value) {
        return SCALA.validateExpression(alertSink, value);
      }
    ...
    private void toAnonymousClosureImpl(Expression value) {
      ...
      appendLine(value.getSourcePosition(),
                 "new " + getAnonymousScalaType(value.getSchema()) + "() {");
